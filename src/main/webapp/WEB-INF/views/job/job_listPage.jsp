        ...
        <div class="nav">
            <ul class="clearfix mainmenu">
                <li>
                	...
                	<a href="${contextPath }/job/job_listPage.do">
                        <h5>배송</h5>
                    </a>
                    <ul class="subMenu">
    ...
    <section class="page-section ">
        ...
        <div>
            <ul>
                ...
                <div class="card-db">
                    <img src="${contextPath }/assets/k1.png" alt="">
                </div>
            </ul>
        </div>
        <div class="sort_wrap">
    ...
    <div class="top-content">
               ...
               <div class="main-content">
                   <div class="board-list">
                       ...
                       <c:forEach var ="job_list" items="${job_list }">
                       		
		                   	 <a href="${contextPath}/company/com_gonggo.do?com_apply_ID=${job_list.com_apply_ID}">
	                                <div class="item">
	                                    <div class="applicant-img"><img class="board-img" src="${contextPath }/assets/${job_list.member_profileImage}" alt=""></div>
	                                    <div class="applicant-title">
	                                        <div>${job_list.member_name }</div>
	                                        <div>${job_list.com_apply_intro }</div>
	                                    </div>
	                                    <div class="applicant-time">
	                                    	<c:choose>
	                                    		<c:when test="${job_list.com_apply_day == 'mon'}">
	                                    			<p>월요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'tue'}">
	                                    			<p>화요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'wed'}">
	                                    			<p>수요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'thu'}">
	                                    			<p>목요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'fri'}">
	                                    			<p>금요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'sat'}">
	                                    			<p>토요일</p> 
	                                    		</c:when>
	                                    		<c:when test="${job_list.com_apply_day == 'sun'}">
	                                    			<p>일요일</p> 
	                                    		</c:when>
	                                    		
	                                    	</c:choose>
	                                    </div> 
	                                    <div class="applicant-location">${job_list.registration_date}</div>
	                                </div>
	                         </a>
	                   </c:forEach>
            ...
            <div>
				 <c:forEach begin="1" end="${totalPage}" var="num">
				    <span>
				     	<a class="h6" href="${contextPath}/job/job_listPage.do?num=${num}" style="margin-right: 2%";>${num}</a>
				  </span>
				 </c:forEach>
				</div>
