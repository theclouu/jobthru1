...
function categoryChange(e) {
    ...
    if (e.value == "a") {
var d = good_a; var r = bad_a ;	
} 
    else if (e.value == "b") {var d = good_b; var r = bad_b;}
    else if (e.value == "c") {var d = good_c; var r = bad_c }
    else if (e.value == "d") {var d = good_d; var r = bad_d }
...
/*지역 카테고리 선택*/
$(function () {
    areaSelectMaker("select[name=per_apply_region1]");
});
...
/*라디오 버튼 선택에 따른 비활성화-시간*/
$(document).ready(function () {
    // 라디오버튼 클릭시 이벤트 발생
    $("input:radio[name=radioTime]").click(function () {
        if ($("input[name=radioTime]:checked").val() == "1") {
            $('#time').attr("disabled", false), $('#time2').attr("disabled", false);
            // radio 버튼의 value 값이 1이라면 활성화

        } else if ($("input[name=radioTime]:checked").val() == "0") {
            $('#time').attr("disabled", true),$('#time2').attr("disabled", true);
